...
// This is the most popular one: https://thebookofshaders.com/10/
...
void main() {
  ...
  // Pattern 8
  // we can use "ternary" or "if" but we should avoid them for performance
  // instead we can use the step function where everything below the
  // threshold (0.5) is 0 and above is 1
  // float strength = mod(vUv.y * 10.0, 1.0);
  ...
  // Used from pattern 3 to 49
  // gl_FragColor = vec4(vec3(strength), 1.0);

  // When using colours we can see strange behaviors
  // at the intersections of #11, #14 and #15.
  // It's because the strength is higher than 1.0 and
  // the output get extrapolated.
  // To fix that we need to clamp the strength
  strength = clamp(strength, 0.0, 1.0);

  // Coloured version
  // Test uncommenting other patterns strength above
  // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 20.0));
